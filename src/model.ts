...
/** Keep the executable effect generated by a weapon mastery in sync with its selection. */
export function syncWeaponMasteryEffects(
  ...
  const template = project.templates.find((item) => item.id === entity.templateId);
  if (
    !masteryParameter ||
    !effectsParameter ||
    !template?.fields.some((item) => item.referenceId === masteryParameter.id) ||
    !template.fields.some((item) => item.referenceId === effectsParameter.id)
  )
    return;

  const current = entity.values[effectsParameter.id] as RuleSet | undefined;
  const manualRules =
    current?.version === 1 && Array.isArray(current.rules)
      ? current.rules.filter(
          (rule) => rule.generatedBy?.kind !== "weapon_mastery",
        )
      : [];
  const selectedId = entity.values[masteryParameter.id];
  const selected =
    typeof selectedId === "string"
      ...
      : undefined;
  const generatedRules = (selected?.ruleSet?.rules ?? []).map((rule) => ({
    ...structuredClone(rule),
    generatedBy: {
      kind: "weapon_mastery" as const,
  ...
  if (!manualRules.length && !generatedRules.length) {
    if (current?.rules.some((rule) => rule.generatedBy?.kind === "weapon_mastery"))
      delete entity.values[effectsParameter.id];
    return;
  }
  entity.values[effectsParameter.id] = {
    version: 1,
    rules: [...manualRules, ...generatedRules],
  } satisfies RuleSet;
}

export function syncAllWeaponMasteryEffects(project: ForgeProject) {
  for (const entity of project.entities)
    syncWeaponMasteryEffects(project, entity);
}

function installWeaponMasteryAutomation(project: ForgeProject) {
  const catalog = buildEquipmentRulesCatalog();
  for (const mastery of project.references.filter(
    (item) => item.kind === "value" && item.optionGroup === "weapon_mastery",
  )) {
    const prepared = catalog.references.find(
      (item) =>
        item.kind === "value" &&
        item.optionGroup === "weapon_mastery" &&
        item.key === mastery.key,
    );
    if (prepared?.ruleSet) mastery.ruleSet = structuredClone(prepared.ruleSet);
  }
}

export function upgradeProjectCatalog(project: ForgeProject): ForgeProject {
  if (project.catalogVersion === 20 && project.ruleEngine) {
    const synchronized = structuredClone(project);
    installWeaponMasteryAutomation(synchronized);
    syncAllWeaponMasteryEffects(synchronized);
    return synchronized;
  }
  ...
  // Schema 20 installs the SRD weapon masteries as prepared reference values.
  // Existing local records and edits are preserved; only missing masteries are added.
  for (const siteMastery of site.references.filter(
    (item) => item.kind === "value" && item.optionGroup === "weapon_mastery",
  )) {
    const exists = next.references.some(
      (item) =>
        item.kind === "value" &&
        item.optionGroup === "weapon_mastery" &&
        item.key === siteMastery.key,
    );
    if (!exists) next.references.push(structuredClone(siteMastery));
  }
  installWeaponMasteryAutomation(next);
  syncAllWeaponMasteryEffects(next);
  next.catalogVersion = 20;
...
function validateRuleSet(
  ...
  for (const rule of ruleSet.rules) {
    ...
    for (let actionIndex = 0; actionIndex < actions.length; actionIndex += 1) {
      ...
      actions.push(...(action.onSuccessActions ?? []), ...(action.onFailureActions ?? []));
      if (
        strict &&

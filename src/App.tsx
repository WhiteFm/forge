...
function TemplatesPage({ project, locale, t, selectedId, setSelectedId, updateProject }: Omit<CollectionProps, "search" | "setSearch">) {
  ...
  const create = () => {
    ...
    updateProject((draft) => { draft.templates.push({ id, type, name: { en: english }, fields: [], previousIds: [] }); });
    setSelectedId(id);
  };
  const remove = () => {
    if (!selected) return;
  ...
  return <><div className="entity-tabs">{ENTITY_TYPES.map((id) => <button key={id} className={type === id ? "active" : ""} onClick={() => { setType(id); setSelectedId(""); }}>{localized(entityNames[id], locale)}<em>{project.templates.filter((item) => item.type === id).length}</em></button>)}</div><div className="info-strip">{t("templateHelp")}</div><CollectionLayout toolbar={<><div className="toolbar-title">{localized(entityNames[type], locale)}</div><button className="button primary" onClick={create}>+ {t("add")}</button></>} list={visible.length ? visible.map((template) => <button key={template.id} className={`flat-record ${selected?.id === template.id ? "selected" : ""}`} onClick={() => setSelectedId(template.id)}><span>{localized(template.name, locale)}</span><small>{template.id}</small></button>) : <div className="empty">{t("noItems")}</div>} editor={selected && selected.type === type ? <Panel title={localized(selected.name, locale)} kicker={selected.id} actions={<button className="icon-button danger" onClick={remove}>×</button>}><TemplateEditor template={selected} project={project} locale={locale} t={t} updateProject={updateProject} /></Panel> : <div className="empty editor-empty">{t("noSelection")}</div>} /></>;
}
...
function EntitiesPage({ project, locale, t, type, setType, search, setSearch, selectedId, setSelectedId, updateProject }: CollectionProps & { type: EntityType; setType: (type: EntityType) => void }) {
  ...
  const create = () => {
    ...
    updateProject((draft) => {
      ...
      const entity: ForgeEntity = { id, key: `${type}_${index}`, type, templateId: chosenTemplate.id, name: { en: `${type[0].toUpperCase()}${type.slice(1)} ${index}` }, values: {}, previousIds: [] };
...
function EntityEditor({ entity, template, templates, project, locale, t, updateProject }: { entity: ForgeEntity; template: EntityTemplate; templates: EntityTemplate[]; project: ForgeProject; locale: Locale; t: (key: LabelKey) => string; updateProject: (recipe: (draft: ForgeProject) => void) => void }) {
  ...
  const changeTemplate = (templateId: string) => updateProject((draft) => {
    ...
    target.values = Object.fromEntries(Object.entries(target.values).filter(([referenceId]) => retained.has(referenceId)));
  });

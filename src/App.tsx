...
const labels = {
  en: {
    ...
    readonlyJson: "Generated JSON is read-only.", valid: "No blocking errors.", version: "Version", namespace: "Namespace", refName: "Reference name", packName: "Pack name", packSubtitle: "Pack label", packDescription: "Pack description", key: "Key", template: "Template",
    ...
    flow1: "Define universal values and dice", flow2: "Build parameters, values and reusable effects", flow3: "Assemble any number of reusable entity templates", flow4: "Create content without writing JSON",
    ...
    templateHelp: "A type can have any number of templates, such as Weapon, Armor and Tool for Items. Template changes preserve entity values whose parameter IDs remain unchanged.", entityHelp: "Choose a template, then create an entity. Entity IDs are generated from namespace, type and English name.",
    createTemplateFirst: "Create at least one template for this entity type first.", confirmTemplateDelete: "Deleting this template will also delete the entities created from it.",
  ...
  ru: {
    ...
    readonlyJson: "Сгенерированный JSON доступен только для чтения.", valid: "Блокирующих ошибок нет.", version: "Версия", namespace: "Пространство имён", refName: "Название справочника", packName: "Название пака", packSubtitle: "Надпись пака", packDescription: "Описание пака", key: "Ключ", template: "Шаблон",
    ...
    flow1: "Определите общие значения и кости", flow2: "Соберите параметры, значения и переиспользуемые эффекты", flow3: "Создайте любое количество переиспользуемых шаблонов", flow4: "Создавайте контент без написания JSON",
    ...
    templateHelp: "Для одного типа можно создать сколько угодно шаблонов: например «Оружие», «Броня» и «Инструмент» для предметов. При изменении шаблона сохраняются значения неизменившихся параметров.", entityHelp: "Выберите шаблон, затем создайте сущность. ID создаётся из пространства имён, типа и английского названия.",
    createTemplateFirst: "Сначала создайте хотя бы один шаблон для этого типа сущности.", confirmTemplateDelete: "Вместе с шаблоном будут удалены созданные по нему сущности.",
  ...
  sv: {
    ...
    readonlyJson: "Genererad JSON är skrivskyddad.", valid: "Inga blockerande fel.", version: "Version", namespace: "Namnrymd", refName: "Referensnamn", packName: "Paketnamn", packSubtitle: "Paketetikett", packDescription: "Paketbeskrivning", key: "Nyckel", template: "Mall",
    ...
    flow1: "Definiera universella värden och tärningar", flow2: "Bygg parametrar, värden och återanvändbara effekter", flow3: "Skapa valfritt antal återanvändbara entitetsmallar", flow4: "Skapa innehåll utan att skriva JSON",
    ...
    templateHelp: "En typ kan ha valfritt antal mallar, till exempel Vapen, Rustning och Verktyg för Föremål. Malländringar bevarar värden med oförändrade parameter-ID:n.", entityHelp: "Välj en mall och skapa sedan en entitet. ID skapas från namnrymd, typ och engelskt namn.",
    createTemplateFirst: "Skapa först minst en mall för denna entitetstyp.", confirmTemplateDelete: "När mallen tas bort tas även entiteter som skapats från den bort.",
...
function App() {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      <button className="brand" onClick={() => changePage("overview")}><img src="/forge-logo.svg" alt="WSGuild Forge" /><span><strong><span>WSGuild</span><span>Forge</span></strong></span></button>
...
function TemplatesPage({ project, locale, t, selectedId, setSelectedId, updateProject }: Omit<CollectionProps, "search" | "setSearch">) {
  ...
  const selected = project.templates.find((item) => item.id === selectedId || item.previousIds.includes(selectedId));
  const create = () => {
    ...
    updateProject((draft) => { draft.templates.push({ id, type, categoryId: "wsg.category.custom", name: { en: english }, fields: [], previousIds: [] }); });
    setSelectedId(id);
  };
  const remove = () => {
    if (!selected) return;
    const linked = project.entities.filter((entity) => entity.templateId === selected.id);
    const message = linked.length ? `${t("confirmTemplateDelete")}\n\n${linked.map((entity) => `• ${localized(entity.name, locale)}`).join("\n")}` : `${t("delete")}: ${localized(selected.name, locale)}?`;
    if (!window.confirm(message)) return;
    updateProject((draft) => {
      draft.templates = draft.templates.filter((item) => item.id !== selected.id);
      draft.entities = draft.entities.filter((entity) => entity.templateId !== selected.id);
    });
    setSelectedId("");
  };
  const groups = project.categories.filter((category) => visible.some((template) => template.categoryId === category.id));
  return <><div className="entity-tabs">{ENTITY_TYPES.map((id) => <button key={id} className={type === id ? "active" : ""} onClick={() => { setType(id); setSelectedId(""); }}>{localized(entityNames[id], locale)}<em>{project.templates.filter((item) => item.type === id).length}</em></button>)}</div><div className="info-strip">{t("templateHelp")}</div><CollectionLayout toolbar={<><div className="toolbar-title">{localized(entityNames[type], locale)}</div><button className="button primary" onClick={create}>+ {t("add")}</button></>} list={visible.length ? groups.map((category) => <details key={category.id} open><summary><span>{localized(category.name, locale)}</span><em>{visible.filter((template) => template.categoryId === category.id).length}</em></summary><div>{visible.filter((template) => template.categoryId === category.id).map((template) => <button key={template.id} className={selected?.id === template.id ? "selected" : ""} onClick={() => setSelectedId(template.id)}><span>{localized(template.name, locale)}</span><small>{template.id}</small></button>)}</div></details>) : <div className="empty">{t("noItems")}</div>} editor={selected && selected.type === type ? <Panel title={localized(selected.name, locale)} kicker={selected.id} actions={<button className="icon-button danger" onClick={remove}>×</button>}><TemplateEditor template={selected} project={project} locale={locale} t={t} updateProject={updateProject} /></Panel> : <div className="empty editor-empty">{t("noSelection")}</div>} /></>;
}
...
function EntitiesPage({ project, locale, t, type, setType, search, setSearch, selectedId, setSelectedId, updateProject }: CollectionProps & { type: EntityType; setType: (type: EntityType) => void }) {
  ...
  const chosenTemplate = templates.find((item) => item.id === templateId) ?? templates[0];
  ...
  const create = () => {
    ...
    updateProject((draft) => {
      ...
      const entity: ForgeEntity = { id, key: `${type}_${index}`, type, templateId: chosenTemplate.id, name: { en: `${type[0].toUpperCase()}${type.slice(1)} ${index}` }, values: Object.fromEntries(chosenTemplate.fields.filter((field) => field.defaultValue !== undefined).map((field) => [field.referenceId, structuredClone(field.defaultValue)])), previousIds: [] };
  ...
  const selectedTemplate = selected ? project.templates.find((item) => item.id === selected.templateId) : undefined;
  return <><div className="entity-tabs">{ENTITY_TYPES.map((id) => <button key={id} className={type === id ? "active" : ""} onClick={() => { setType(id); setTemplateId(""); setSelectedId(""); }}>{localized(entityNames[id], locale)}<em>{project.entities.filter((item) => item.type === id).length}</em></button>)}</div><div className="info-strip">{templates.length ? t("entityHelp") : t("createTemplateFirst")}</div><CollectionLayout toolbar={<><input className="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder={t("search")} />{templates.length > 0 && <label className="toolbar-select"><span>{t("template")}</span><select value={chosenTemplate?.id ?? ""} onChange={(event) => setTemplateId(event.target.value)}>{templates.map((template) => <option key={template.id} value={template.id}>{localized(template.name, locale)}</option>)}</select></label>}<button className="button primary" disabled={!chosenTemplate} onClick={create}>+ {t("add")}</button></>} list={filtered.length ? filtered.map((item) => <button key={item.id} className={`flat-record ${item.id === selectedId ? "selected" : ""}`} onClick={() => setSelectedId(item.id)}><span>{localized(item.name, locale)}</span><small>{item.id}</small></button>) : <div className="empty">{t("noItems")}</div>} editor={selected && selectedTemplate ? <Panel title={localized(selected.name, locale)} kicker={selected.id} actions={<button className="icon-button danger" onClick={remove}>×</button>}><EntityEditor entity={selected} template={selectedTemplate} templates={templates} project={project} locale={locale} t={t} updateProject={updateProject} /></Panel> : <div className="empty editor-empty">{t("noSelection")}</div>} /></>;
}

function EntityEditor({ entity, template, templates, project, locale, t, updateProject }: { entity: ForgeEntity; template: EntityTemplate; templates: EntityTemplate[]; project: ForgeProject; locale: Locale; t: (key: LabelKey) => string; updateProject: (recipe: (draft: ForgeProject) => void) => void }) {
  ...
  const changeTemplate = (templateId: string) => updateProject((draft) => {
    const target = draft.entities.find((item) => item.id === entity.id)!;
    const nextTemplate = draft.templates.find((item) => item.id === templateId)!;
    const retained = new Set(nextTemplate.fields.map((field) => field.referenceId));
    target.templateId = templateId;
    const preserved = Object.fromEntries(Object.entries(target.values).filter(([referenceId]) => retained.has(referenceId)));
    target.values = { ...Object.fromEntries(nextTemplate.fields.filter((field) => field.defaultValue !== undefined).map((field) => [field.referenceId, structuredClone(field.defaultValue)])), ...preserved };
  });
  return <div className="editor-stack"><div className="form-grid two"><TextField label={t("id")} value={entity.id} onChange={() => {}} disabled /><SelectField label={t("template")} value={entity.templateId} onChange={changeTemplate}>{templates.map((item) => <option key={item.id} value={item.id}>{localized(item.name, locale)}</option>)}</SelectField></div><LocalizedFields value={entity.name} t={t} onChange={(name) => updateProject((draft) => { draft.entities.find((item) => item.id === entity.id)!.name = name; })} />{template.fields.length === 0 ? <div className="empty">{t("templateHelp")}</div> : <div className="dynamic-form">{template.fields.map((field) => { const parameter = project.references.find((item) => item.id === field.referenceId); if (!parameter) return <div className="missing" key={field.id}>Missing: {field.referenceId}</div>; const value = entity.values[field.referenceId]; return <DynamicField key={field.id} parameter={parameter} value={value} locale={locale} project={project} required={field.required} multiple={field.multiple} onChange={(next) => setValue(field.referenceId, next)} />; })}</div>}</div>;
}
